...
function StudentDashboard() {
    ...
    const handleDownloadCertificate = async () => {
        try {
            toast.loading('Preparing your certificate...', { id: 'cert' });
            const response = await api.get('/student/certificate', { responseType: 'blob' });
            const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', 'Babcock_Clearance_Certificate.pdf');
            ...
            toast.error('Could not generate certificate. Try again.', { id: 'cert' });
        }
    };
            ...
            <div className="page-enter" style={{ maxWidth: 1100, margin: '0 auto', width: '100%', padding: 'var(--space-8) var(--space-6)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 350px) 1fr', gap: 'var(--space-6)', alignItems: 'start' }}>
                    ...
                    <div className="glass-card" style={{ padding: 'var(--space-8)' }}>
                        ...
                        {clearance && (
                            <div>
                                {/* Clearance Complete — Download Certificate */}
                                {isCleared && (
                                    <div className="glass-card" style={{
                                        background: 'var(--success-soft)',
                                        border: '1px solid rgba(16,185,129,0.2)',
                                        marginBottom: 'var(--space-5)', padding: 'var(--space-6)',
                                        textAlign: 'center',
                                    }}>
                                        <div style={{ fontSize: 40, marginBottom: 'var(--space-2)' }}>🎓</div>
                                        <h4 style={{ fontWeight: 800, color: 'var(--success)', marginBottom: 4, fontSize: 'var(--text-lg)' }}>
                                            Clearance Complete!
                                        </h4>
                                        <p style={{ color: 'var(--text-muted)', fontSize: 'var(--text-sm)', marginBottom: 'var(--space-4)' }}>
                                            All departments have cleared you. Download your official certificate below.
                                        </p>
                                        <button onClick={handleDownloadCertificate} className="btn-success-glass" style={{ padding: 'var(--space-3) var(--space-8)', fontSize: 'var(--text-base)', fontWeight: 700 }}>
